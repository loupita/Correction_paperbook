...
<body>
	<%
		...
		Collection<Conference> propositions = (Collection<Conference>) session.getAttribute("conferences");
	%>
	<!-- Header -->
	<!-- Fixed navbar -->
	...
	<div class=container>
		...
		<div class="col-md-9">
			...
			<h3>Propositions :</h3>
			<ul>
				<%for(Conference c : propositions) {
				String rang =c.getRang();
				String titre = c.getTitre();
				String dateAbstr = c.getDateAbstractSubmission();
				%>
				<li><%=titre%> <b>(<%=rang%>)
				</b> : <mark><%=dateAbstr%></mark></li>
				<%}%>
			</ul>
			<%}%>
		</div>
